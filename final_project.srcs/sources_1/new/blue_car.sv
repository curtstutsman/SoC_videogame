module  blue_car ( input logic blue_car_set, sprite_clk, rom_clk,
               input logic [9:0] DrawX, DrawY,
               input logic [7:0] keycode0,
               input logic [15:0] seed,
               input logic reset,
               input logic [3:0] state_variable,
               output logic blue_car_on,
               output logic [11:0] blue_car_color,
               output logic blue_car_point
               );
...
//for the row address, do the same thing just in the y dimension, then just multiply by the size of a row           
    ...
    blue_car_palette blue_car_palette (
        .index (rom_q),
    ...
    obstacle_mover blue_car_mover (
     ...
     always_comb begin
        if (DrawX >= X_Coordinate && DrawY >= Y_Coordinate
            ...
            && rom_q!=1 && rom_q!=10 && rom_q!=11 && rom_q!=13)                    //Will have to change this line for each sprite depending on color to ignore
            blue_car_on = 1'b1;
        else
            blue_car_on = 1'b0; 
     end

module  yellow_car ( input logic yellow_car_set, sprite_clk, rom_clk,
               input logic [9:0] DrawX, DrawY,
               input logic [7:0] keycode0,
               input logic [15:0] seed,
               input logic reset,
               input logic [3:0] state_variable,
               output logic yellow_car_on,
               output logic [11:0] yellow_car_color,
               output logic yellow_car_point
               );
...
//for the row address, do the same thing just in the y dimension, then just multiply by the size of a row           
    ...
    yellow_car_palette yellow_car_palette (
        .index (rom_q),
    ...
    obstacle_mover yellow_car_mover (
        .clk(sprite_clk),
        .keycode(keycode0),
        .reset(reset),
        .set(yellow_car_set),
     ...
     always_comb begin
        if (DrawX >= X_Coordinate && DrawY >= Y_Coordinate
            ...
            && rom_q!=2 && rom_q!=11 && rom_q!=12 && rom_q!=13)                    //Will have to change this line for each sprite depending on color to ignore
            yellow_car_on = 1'b1;
        else
            yellow_car_on = 1'b0; 
     end

...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
			...
			Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
			if(Status != XST_SUCCESS) {
				xil_printf("SetSlaveSelect Success\n");
			}
			BYTE WriteBuffer[2] = {(BYTE) reg+2, val};
			...
			Status = XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x0);
			if(Status != XST_SUCCESS) {
				xil_printf("There's a problem with deslecting the slave in MAXreg_wr\n");
			}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
		...
		Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
		if(Status != XST_SUCCESS) {
			xil_printf("There's a problem with selecting the slave in MAXbytes_wr\n");
		}
		//write reg + 2 via SPI'
		//xil_printf("data in maxbytes_wr is %d\n", data);
		//write data[n] via SPI, where n goes from 0 to nbytes-1

		BYTE WriteBuffer[nbytes+1];
		WriteBuffer[0] = (BYTE) reg+2;
		...
		if(Status != XST_SUCCESS) {
			xil_printf("There's a problem with SPI Transfer in Maxbytes_wr\n");
		}
		...
		Status = XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x0);
		if(Status != XST_SUCCESS) {
			xil_printf("There's a problem with deselecting the slave in maxbytes_wr\n");
		}
		//return (data + nbytes);
		return (data+nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
		...
		Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
		if(Status != XST_SUCCESS) {
			xil_printf("Problem with selecting slave\n");
		}

		BYTE ReadBuffer[2] = {(BYTE) reg, 0x00};

		Status = XSpi_Transfer(&SpiInstance, ReadBuffer, ReadBuffer, 2);
		if(Status != XST_SUCCESS) {
			xil_printf("Problem with data transfer\n");
		}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
		...
		Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
		if(Status != XST_SUCCESS) {
			xil_printf("Success with selecting slave in maxbytes_rd\n");
		}

		BYTE ReadBuffer[nbytes+1];
		ReadBuffer[0] = (BYTE) reg;
		//write reg via SPI
		//read data[n] from SPI, where n goes from 0 to nbytes-1
		Status = XSpi_Transfer(&SpiInstance, ReadBuffer, ReadBuffer, nbytes+1);
		if(Status != XST_SUCCESS) {
			xil_printf("Problem with data transfer in maxbytes_rd\n");
		}
		...
		Status = XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x0);
		if(Status != XST_SUCCESS) {
			xil_printf("Problem with slave deselect in maxbytes_rd\n");
		}
		//read return code from SPI peripheral
		//if return code != 0 print an error
		//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
